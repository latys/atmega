...
/***************************************1015.4.2 修改********************************************************************
将motor1,motor2 合并为motor1_2,所有相关参数都改为了motor1_2.*/
...
unsigned char global_yon_motor1_2_running;
...
void motor1_2_start(char direct);
void motor1_2_stop();
...
//	TCNT1L=0xe7;          //reload 
    ...
    if(global_yon_motor1_2_running==TRUE)
	{
		motor1_2_Step++;
    	PORTB^=0x60;          //取反生成pwm波
	}
...
{
	motor5Step++;
	if(Motor5Status==MOTOR_FORWARD)
	{
		Position5++;
	}
	else if(Motor5Status==MOTOR_BACKWARD)
	{
		Position5--;
	}
...
void motor1_2_start(char direct)
{
	...
	//	PORTL |=(1<<3);
		...
		TIMSK1	&=~(1<<TOIE1);            //禁止溢出中断
	//	PORTB &=~(1<<5);
     	PORTB |=(1<<5);
		PORTB |=(1<<6);

		delayus(5);
		TIMSK1	|=(1<<TOIE1);            //允许溢出中断
		...
		if(UART0_RECV_FLAG==1)
	{
		...
		switch(motor_command.MotorStart)
		{
			case START:
			{
				...
				{
					case MOTOR1_2:
					global_yon_motor1_2_running=TRUE;
					motor1_2_start(motor_command.MotorDirection);
					Motor1_2_Status=motor_command.MotorDirection;
					memcpy(Motor1_2_CommandStep,&motor_command.MotorStep,sizeof(motor_command.MotorStep));
					motor1_2_CommandStep=atol(Motor1_2_CommandStep);
			...
			case STOP:
			{
				switch(motor_command.MotorID)
				{
					case MOTOR1_2:
					motor1_2_stop();
					global_yon_motor1_2_running=FALSE;
					Motor1_2_Status=MOTOR_STOP;
					motor1_2_Step=0;
...
void handleAction(char buffer)
{
	...
	else if((buffer&0b11000000)==0b11000000)
	{
		...
		motor5_speed(0x3FF/32*(buffer&0b00011111));
   ...
   /* if (global_yon_motor1_running==TRUE) {
	 	...
	 	if(motor5Step>=motor5CommandStep-fmin(motor5CommandStep*0.05,SLOW_STEP)&&motor5Step<motor5CommandStep)
	{
     ...
     if (global_yon_motor6_running==TRUE) {
	 	if(motor6Step>=motor6CommandStep-fmin(motor6CommandStep*0.05,SLOW_STEP)&&motor6Step<motor6CommandStep)
	{
		motor6_slow();
	 }
	 }
	 ...
	 if (global_yon_motor1_2_running==TRUE) {
		if(motor1_2_CommandStep<=motor1_2_Step)
	{
		motor1_2_stop();
        global_yon_motor1_2_running=FALSE;
		motor1_2_Step=0;
...
/*	if((PINK&0x01)==0||(PINK&0x02)==0)
	...
	if(((PINK&0x04)==0&&Motor3Status==MOTOR_FORWARD)||((PINK&0x08)==0&&Motor3Status==MOTOR_BACKWARD))
	{
		motor3_stop();
        global_yon_motor3_running=FALSE;
		motor3Step=0;
